...
from werkzeug.security import generate_password_hash, check_password_hash  # retained for backward-compat if needed
import hashlib
import os
import base64
import secrets
...
# For Solana wallet signature verification
...
def initialize_key(user_id):
    ...
    key_entry = Key(key_value=new_key.decode(), user_id=user_id)
    try:
...
def get_cipher_suite(user_id):
    return Fernet(initialize_key(user_id))
...
STYLE_TEMPLATE = """
body{background:#1e1e1e;color:#fff;font-family:'Nunito Sans',sans-serif;line-height:1.6;margin:0;min-height:100vh;display:flex;flex-direction:column}
.container{width:95%;max-width:1200px;margin:0 auto;padding:20px;flex:1}
.form-container{width:95%;max-width:600px;margin:2rem auto;padding:2rem;display:flex;flex-direction:column;align-items:center}
.form-container form{width:100%;display:flex;flex-direction:column;align-items:center;gap:1rem}
.button-wrapper{width:100%;display:flex;justify-content:center;margin:1rem 0}
textarea,input[type="text"],input[type="password"],input[type="email"]{width:100%;padding:15px;font-size:16px;border-radius:5px;background:#2d2d2d;color:#fff;border:1px solid #444;transition:border-color .3s}
textarea{height:10vh;resize:vertical}
textarea:focus,input:focus{border-color:#06f;outline:none}
button{width:100%;max-width:300px;padding:15px;font-size:16px;border-radius:5px;background:#06f;color:#fff;border:0;cursor:pointer;transition:background-color .3s}
button:hover{background:#05c}
.menu{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin:20px 0}
.menu form{margin:0}
.menu input[type="file"]{display:none}
.menu button{margin:0;padding:8px 16px;white-space:nowrap}
.auth-container{width:100%;max-width:420px;margin:2rem auto;padding:2rem}
.navbar{background:#1e1e1e;border-bottom:1px solid #444;padding:1rem;width:100%;box-shadow:0 2px 4px rgba(0,0,0,.2)}
.navbar-container{max-width:1200px;margin:0 auto;display:flex;justify-content:space-between;align-items:center}
.navbar-brand{font-size:1.5rem;font-weight:bold;color:#fff;cursor:pointer;user-select:none;padding:.5rem}
.navbar-menu{background:#1e1e1e;border-bottom:1px solid #444;padding:1rem;display:none}
.navbar-menu.active{display:block}
.main-content{padding:2rem 1rem;flex:1}
.badge{font-size:.85rem;padding:.3rem .5rem;border:1px solid #444;border-radius:6px;color:#bbb}
@media(max-width:768px){.container{width:95%;padding:10px}.menu{flex-direction:column;align-items:stretch}.menu button{width:100%;margin:5px 0}.form-container{padding:1rem}}
"""

HEADER_TEMPLATE = """
<div class="header">
  <div style="text-align:center;font-family:'Helvetica',sans-serif;color:#999;">
    <p style="font-size:1.1em;margin:.5em 0;">
      <span style="font-family:'Consolas',monospace;">© 2025</span>
      All rights reserved by
      <span style="font-family:'Consolas',monospace;color:#06f;">imaclone.x</span>
    </p>
  </div>
</div>
"""

# When authenticated, show app menu. When not, show wallet-connect UI.
APP_TEMPLATE = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    {{% if session.get('user_id') %}}
      <nav class="navbar">
        <div class="navbar-container">
          <div class="navbar-brand" onclick="toggleMenu()">☰ Zencrypt</div>
          <div class="badge">Wallet: {{{{ session.get('wallet_address','') }}}}</div>
      ...
      <hr style="border:0;height:1px;background-image:linear-gradient(to right, rgba(0,102,255,0), rgba(0,102,255,.75), rgba(0,102,255,0));">
      {{{{ content | safe }}}}
      {{% if output %}}<div class="output">{{{{ output }}}}</div>{{% endif %}}
    {{% else %}}
      <div class="auth-container">
        <h2>Connect Wallet</h2>
        <p style="color:#bbb;margin-top:-8px">Authenticate with your Solana wallet.</p>
        <div id="wallet-ui">
          <button id="connect-wallet">Connect Phantom</button>
          <p id="status" style="margin-top:12px;color:#9aa"></p>
        </div>
      </div>
      <hr style="border:0;height:1px;background-image:linear-gradient(to right, rgba(0,102,255,0), rgba(0,102,255,.75), rgba(0,102,255,0));">
      ...
      <div class="links">
        <h6 style="text-align:center;">
          ...
          <li><b>ePortfolio</b> - <a href="https://www.imaclone.x.com" target="_blank">https://www.imaclone.x.com</a></li>
        </h6>
      </div>
      <hr style="border:0;height:1px;background-image:linear-gradient(to right, rgba(0,102,255,0), rgba(0,102,255,.75), rgba(0,102,255,0));">
      <script>
        async function connectAndAuth() {{
          ...
          try {{
            if (!window.solana || !window.solana.isPhantom) {{
              status.textContent = 'Phantom not found. Install Phantom and refresh.';
              return;
            }}
            // 1) Connect wallet
            ...
            const message = enc.encode(nonce);

            // 3) Sign nonce
            if (!window.solana.signMessage) {{
              status.textContent = 'signMessage not available in this wallet.';
              return;
            }}
            const signed = await window.solana.signMessage(message, 'utf8');
            const signature_b64 = btoa(String.fromCharCode(...signed.signature));
            ...
            const verifyRes = await fetch('/auth/verify', {{
              ...
              credentials: 'include',
              body: JSON.stringify({{ public_key: pubkey, signature_b64, nonce }})
            }});
            if (!verifyRes.ok) {{
              const txt = await verifyRes.text();
              status.textContent = 'Auth failed: ' + txt;
              return;
            }}
        ...
        document.getElementById('connect-wallet').addEventListener('click', connectAndAuth);
        ...
        window.addEventListener('scroll', function() {{
          ...
          if (menu && currentScroll > lastScroll) menu.classList.remove('active');
...
def auth_verify():
    ...
    try:
        ...
        VerifyKey(pubkey_bytes).verify(message, signature, encoder=RawEncoder)
    except (BadSignatureError, ValueError, Exception) as e:
        return "Signature verification failed", 400

    # Upsert user by wallet address (requires User.wallet_address unique column)
    user = User.query.filter_by(wallet_address=public_key_b58).first() if hasattr(User, 'wallet_address') else None
    if not user:
        try:
            if hasattr(User, 'wallet_address'):
                user = User(wallet_address=public_key_b58)
            else:
                # Fallback: create using email field to store address if schema lacks wallet_address
                user = User(email=f"{public_key_b58}@wallet.local", password_hash=generate_password_hash(secrets.token_hex(16)))
            db.session.add(user)
            db.session.commit()
        except Exception as e:
            db.session.rollback()
...
# ---------------------- | App: Hash / Encrypt / Decrypt / Files | ---------------------- #
...
def hash_page():
    ...
    <div class="form-container">
      <form method="POST">
        ...
        <div class="button-wrapper"><button type="submit">Generate Hash</button></div>
      </form>
    </div>
    """
    if request.method == 'POST':
        ...
        if text:
            ...
            new_hash = Hash(hash_value=hash_value, salt=salt, user_id=session['user_id'])
            db.session.add(new_hash)
            db.session.commit()
            return render_template_string(APP_TEMPLATE, content=content, output=f"SHA256 Hash:\n{hash_value}")
...
def encrypt_page():
    ...
    <div class="form-container">
      <form method="POST">
        ...
        <div class="button-wrapper"><button type="submit">Encrypt</button></div>
      </form>
    </div>
    """
    if request.method == 'POST':
        ...
        if text:
            try:
                ...
                new_encrypted = EncryptedText(encrypted_content=encrypted.decode(), user_id=session['user_id'])
                db.session.add(new_encrypted)
                db.session.commit()
                return render_template_string(APP_TEMPLATE, content=content, output=f"Encrypted Text:\n{encrypted.decode()}")
            except Exception as e:
                db.session.rollback()
                return render_template_string(APP_TEMPLATE, content=content, output=f"Error: {str(e)}")
    return render_template_string(APP_TEMPLATE, content=content)
...
def decrypt_page():
    ...
    <div class="form-container">
      <form method="POST">
        ...
        <div class="button-wrapper"><button type="submit">Decrypt</button></div>
      </form>
    </div>
    """
    if request.method == 'POST':
        ...
        if text:
            try:
                ...
                return render_template_string(APP_TEMPLATE, content=content, output=f"Decrypted Text:\n{decrypted.decode()}")
            except Exception as e:
                return render_template_string(APP_TEMPLATE, content=content, output=f"Error: {str(e)}")
    return render_template_string(APP_TEMPLATE, content=content)
...
def file_page():
    ...
    <div class="form-container">
      <form method="POST" enctype="multipart/form-data" style="text-align:center;">
        <div class="file-upload-wrapper" style="margin:20px 0;">
          <label for="file-upload" class="custom-file-upload" style="display:inline-block;padding:10px 20px;background:#2d2d2d;color:#fff;border:1px solid #444;border-radius:5px;cursor:pointer;margin-bottom:10px;">Choose File</label>
        ...
        <select name="operation" style="width:75%;padding:15px;margin:20px 0;background-color:#2d2d2d;color:#fff;border:1px solid #444;border-radius:5px;">
          <option value="encrypt">Encrypt File</option>
          <option value="decrypt">Decrypt File</option>
        </select>
        <div class="button-wrapper"><button type="submit">Process File</button></div>
      </form>
    </div>
    <script>
    ...
    if request.method == 'POST':
        if 'file' not in request.files:
            return render_template_string(APP_TEMPLATE, content=content, output="Please select a file to process")
        file = request.files['file']
        if file.filename == '':
            return render_template_string(APP_TEMPLATE, content=content, output="No file selected")
        ...
        try:
            ...
            if not password:
                return render_template_string(APP_TEMPLATE, content=content, output="Password is required")

            cipher_suite = get_cipher_suite(session['user_id'])
            if operation == 'encrypt':
                encrypted = cipher_suite.encrypt(file_content)
                return render_template_string(APP_TEMPLATE, content=content, output=f"File encrypted successfully!\nEncrypted content:\n{base64.b64encode(encrypted).decode()}")
            else:
                try:
                    decrypted = cipher_suite.decrypt(base64.b64decode(file_content))
                    return render_template_string(APP_TEMPLATE, content=content, output=f"File decrypted successfully!\nDecrypted content:\n{decrypted.decode()}")
                except Exception:
                    return render_template_string(APP_TEMPLATE, content=content, output="Invalid encrypted file or wrong password")
        except Exception as e:
            return render_template_string(APP_TEMPLATE, content=content, output=f"Error processing file: {str(e)}")

    return render_template_string(APP_TEMPLATE, content=content)

# ---------------------- | Key Import/Export | ---------------------- #

@app.route('/export-key', methods=['GET'])
def export_key():
    if not require_session():
        return redirect(url_for('auth_page'))
    try:
        ...
        if key:
            key_name = request.args.get('key_name', 'zen_key')
...
def import_key():
    ...
    try:
        ...
        new_key = Key(key_value=key_content, user_id=session['user_id'], active=True)
...
def pgp_page():
    ...
    <div class="form-container">
      <form method="POST" action="/pgp/generate">
        <div class="button-wrapper"><button type="submit">Generate Keys</button></div>
      </form>
...
def generate_pgp():
    ...
    try:
        ...
        new_keys = PGPKey(public_key=public_key, private_key=private_key, user_id=session['user_id'])
...
def pgp_encrypt():
    ...
    try:
        ...
        return render_template_string(APP_TEMPLATE, content="Encrypted message:<br><textarea readonly>%s</textarea>" % encrypted)
    except Exception as e:
...
def pgp_decrypt():
    ...
    try:
        ...
        return render_template_string(APP_TEMPLATE, content="Decrypted message:<br><textarea readonly>%s</textarea>" % decrypted)
    except Exception as e:
        return f"Error decrypting message: {str(e)}", 500

# ---------------------- | Advanced (session-gated) | ---------------------- #
...
def advanced_crypto():
    ...
    return render_template_string(APP_TEMPLATE, content="Advanced Cryptography Options")
...
def print_startup_banner():
    print(ASCII_BANNER)
    print(f"{BOLD}Zencrypt Web-App{END} - Developed And Owned Entirely By imaclone.x{END}\n")
    print(f"Zencrypt {BOLD}Whitepapers and Docs{END} - {BOLD}https://zencrypt.gitbook.io/zencrypt{END}")
    print(f"{BOLD}ePortfolio{END} - {BOLD}https://www.imaclone.x.com{END}\n")
    print(f"Thank you for using Zencrypt {BOLD}v6.3-WALLET{END}\n")
...
#                     </div>
#                     <div class="navbar-menu" id="navMenu">
#                         <a href="/connect-wallet"><button>Connect Wallet</button></a>
#                         <a href="/"><button>Generate Hash</button></a>
#                         <a href="/encrypt"><button>Encrypt Text</button></a>
#                         <a href="/decrypt"><button>Decrypt Text</button></a>
#                         <a href="/file"><button>Encode / Decode .txt Files</button></a>
...
#             <h3>Select Plan</h3>
#             <button onclick="selectPlan('personal')">Personal ($50)</button>
#             <button onclick="selectPlan('pro')">Pro ($150)</button>
#             <button onclick="selectPlan('team')">Team ($1000)</button>
#         </div>
#     </div>
